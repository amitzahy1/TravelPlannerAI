    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden group/card relative">
            ...
            {/* ── Card header — square image on the right, info column next,
                 ...
                 hotel name was capped at 180 px and edit/delete ate mobile
                 screen space. ── */}
            <div
                className="flex items-stretch gap-3 p-3 cursor-pointer hover:bg-slate-50/70 transition-colors select-none relative"
                onClick={() => setIsExpanded(v => !v)}
            >
                {/* Square thumbnail (RTL: visual right) */}
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-slate-100 shrink-0 relative">
                    ...
                    {group.hotels.length > 1 && (
                        <span className="absolute top-1.5 right-1.5 bg-indigo-600 text-white text-2xs font-black px-1.5 py-0.5 rounded-pill shadow">
                            ×{group.hotels.length}
                        </span>
                    )}
                </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                    {/* Name — no truncate, wraps naturally */}
                    <h3 className="font-bold text-sm sm:text-base text-slate-900 leading-snug pr-10">
                        {primary.name}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-2xs text-slate-500">
                        {primary.city && (
                            <span className="inline-flex items-center gap-1">
                                ...
                                <span className="font-semibold">{primary.city}</span>
                            </span>
                        )}
                        {primary.checkInDate && primary.checkOutDate && (
                            <span className="font-semibold" dir="ltr">
                                {formatDate(primary.checkInDate)} → {formatDate(primary.checkOutDate)}
                            </span>
                        )}
                    </div>
