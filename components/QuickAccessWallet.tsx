...
import { X, Plane, Hotel, Copy, Eye, EyeOff, ShieldCheck, Plus, Trash2, MapPin, Calendar, Lock, Maximize2 } from 'lucide-react';
...
export const QuickAccessWallet: React.FC<QuickAccessWalletProps> = ({ trip, onClose, onUpdateTrip }) => {
  ...
  const [showDriverMode, setShowDriverMode] = useState<string | null>(null); // Holds address string
      ...
      <div className="bg-slate-100 w-full md:max-w-md h-[90vh] md:h-[650px] rounded-t-[2rem] md:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative">
        ...
        <div className="bg-slate-900 text-white p-6 pb-8 rounded-b-[2rem] shadow-lg relative z-10 flex-shrink-0">
            ...
            <div className="flex bg-slate-800/50 p-1 rounded-xl">
                <button onClick={() => setActiveTab('cards')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'cards' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-400'}`}>כרטיסים ושוברים</button>
                <button onClick={() => setActiveTab('vault')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'vault' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-400'}`}>כספת מאובטחת</button>
            </div>
        </div>
